...
class FileOperation {
	...
	boolean saveFile(File temp) {
		...
		TextEncrypt te = new TextEncrypt();
		try {
			// fout=new FileWriter(temp);

			// run through the encrypter here.
			byte[] s = te.encrypt(npd.ta.getText());
			te.FileOutputStreamToFile(s, temp);
	...
	boolean openFile(File temp) {
		...
		try {
			...
			// din=fin.readAllBytes();//new BufferedReader(new InputStreamReader(fin));
			din = te.FileInputStreamFromFile(temp);

			String str = te.decrypt(din);
			// while(str!=null)
			// {
			// str=din.readLine();
			// bin = din.re
			// str = te.decrypt(str.getBytes());
			// run through the decrypter here.
			// if(str==null)
			// break;
